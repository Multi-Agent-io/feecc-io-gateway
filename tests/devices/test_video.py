...
def test_start_second_record() -> None:
    ...
    records_list_resp = test_client.get("/video/records")
    assert len(
        records_list_resp.json().get("ongoing_records")) != 0, "second recording hasn't started"
    assert len(
        records_list_resp.json().get("ended_records")) == 1, "second recording hasn't started"
...
def test_stop_second_record() -> None:
    ...
    assert len(records_list_resp.json().get("ongoing_records")) == 0, "recording hasn't stopped"
    assert len(
        records_list_resp.json().get("ended_records")) == 2, "second recording hasn't started"
...
def test_multiple_records_cycle() -> None:
    ...
    end_second_rec_resp = test_client.post("/video/camera/1/stop", json={
        "record_id": first_rec_resp.json().get("record_id")})
